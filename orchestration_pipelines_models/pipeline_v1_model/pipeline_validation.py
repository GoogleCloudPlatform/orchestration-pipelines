...
class PipelineValidator:
    ...
    def _validate_dataset_trigger(cls, dataset_trigger: Message, path: str):
        ...
        if dataset_trigger.condition:
            if dataset_trigger.condition.lower() not in ("all", "any"):
                raise ValueError(
                    f"Error for field '{path}.condition': "
                    f"value '{dataset_trigger.condition}' is not valid. "
    ...
    def _validate_pipeline_level_rules(cls, pipeline: Message):
        """Performs pipeline-level validation for triggers, action uniqueness
        and dependencies.
        """
        ...
        if pipeline.triggers:
            ...
            if "schedule" in trigger_types and "datasets" in trigger_types:
                raise ValueError(
                    "A pipeline cannot configure both 'schedule' and "
                    "'datasets' triggers simultaneously."
                )
